<p class="mt-2 text-center dark:text-white">Menampilkan <b class="text-black dark:text-gray-300">{{ $paginator->count() }}</b> Dari <b
    class="text-black dark:text-gray-300">{{ $paginator->total() }}</b> Data</p>
            ...
            @if (is_array($element))
                ...
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="py-2 bg-slate-700 px-4 text-white rounded-md cursor-pointer">{{ $page }}
                        </li>
                    @else
